...
class RESTClient {
    ...
    function post($params) 
    {
        if ( $this->account_type == 'pro' ) {
          $posturl = $this->api_url . "" . $this->endpoint;
        } else {
          $posturl = $this->api_url . "" . $this->account_type . "/" . $this->endpoint;
        }
        $curl = curl_init();
    ...
    function get($params) 
    {
        ...
        if ( $this->account_type == 'pro' ) {
          $geturl = $this->api_url . "" . $this->endpoint . "?" . $query;
        } else {
          $geturl = $this->api_url . "" . $this->account_type . "/" . $this->endpoint . "?" . $query;
        }
        curl_setopt($curl, CURLOPT_HTTPHEADER, $header);
