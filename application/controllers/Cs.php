...
class Cs extends CI_Controller {
	...
	public function samplingjson() {
		...
		foreach ($list as $field) {
			...
			{
				if(($field->LSAM_DELDATE==null)){
					$row[] = '<div style="vertical-align: middle; text-align: center;"><a href="'.$url.'cs/edit_sampling/'.$field->LSAM_ID.'" class="btn btn-primary btn-sm"><i class="fa fa-pen"></i></a>';
				} else {
					$row[] = '<div style="vertical-align: middle; text-align: center;"><a href="'.$url.'cs/edit_sampling/'.$field->LSAM_ID.'" class="btn btn-primary btn-sm"><i class="fa fa-pen"></i></a>
					<a href="'.$url.'cs/sampling_followup/'.$field->CLOG_ID.'" class="btn btn-warning btn-sm" title="Follow Up"><i class="fa fa-share"></i></a></div>';
				}
			} else {
				if(($field->LSAM_DELDATE==null)){
					...
					$row[] = '<form action="'.$url.'cs/del_sampling'.'" method="post"><div style="vertical-align: middle; text-align: center;"><a href="'.$url.'cs/edit_sampling/'.$field->LSAM_ID.'" class="btn btn-primary btn-sm"><i class="fa fa-pen"></i></a>
						...
						<a href="'.$url.'cs/sampling_followup/'.$field->CLOG_ID.'" class="btn btn-warning btn-sm" title="Follow Up"><i class="fa fa-share"></i></a></div></form>';
				}
			}
			$data[] = $row;
		}
		...
		foreach ($list as $field) {
			...
			if((!$this->access_m->isDelete('Check Stock CS', 1)->row()) && ($this->session->GRP_SESSION !=3)){
				if($field->LSTOCK_STATUS==null) {
					...
					$row[] = '<div style="vertical-align: middle; text-align: center;"><a href="'.$url.'cs/edit_check/'.$field->LSTOCK_ID.'" class="btn btn-primary btn-sm"><i class="fa fa-pen"></i></a>
						<a href="'.$url.'cs/check_stock_followup/'.$field->CLOG_ID.'" class="btn btn-warning btn-sm" title="Follow Up"><i class="fa fa-share"></i></a></div>';
				}
			} else {
				if($field->LSTOCK_STATUS==null) {
					...
					$row[] = '<form action="'.$url.'cs/del_stock'.'" method="post"><div style="vertical-align: middle; text-align: center;"><a href="'.$url.'cs/edit_check/'.$field->LSTOCK_ID.'" class="btn btn-primary btn-sm"><i class="fa fa-pen"></i></a>
						...
						<input type="hidden" name="CLOG_ID" value="'.$field->CLOG_ID.'">
						<button onclick="'."return confirm('Hapus data?')".'" type="submit" class="btn btn-danger btn-sm"><i class="fa fa-trash"></i></button>
						<a href="'.$url.'cs/check_stock_followup/'.$field->CLOG_ID.'" class="btn btn-warning btn-sm" title="Follow Up"><i class="fa fa-share"></i></a></div></form>';
				}
			}
			$data[] = $row;
		}
