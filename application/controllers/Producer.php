...
class Producer extends CI_Controller {
	...
	public function producerjson() {
		...
		foreach ($list as $field) {
			...
			$row[] = $PHONE.$EMAIL;
			$row[] = "<div align='center'>$field->PRDUC_NAME</div>";
			$row[] = "<div align='center'>$field->PRDUT_NAME</div>";
			...
			if((!$this->access_m->isDelete('producer', 1)->row()) && ($this->session->GRP_SESSION !=3)){
				$row[] = '<div style="vertical-align: middle; text-align: center;"><a href="'.$url.'producer/edit/'.$field->PRDU_ID.'" class="btn btn-primary btn-sm"><i class="fa fa-pen"></i></a>
					<br><a href="'.$url.'producer/product/'.$field->PRDU_ID.'" class="btn btn-info btn-sm"><i class="fa fa-plus-square"></i> Product</a>
				</div>';
			} else {
				$row[] = '<form action="'.$url.'producer/del" method="post"><div style="vertical-align: middle; text-align: center;">
						...
						<a href="'.$url.'producer/product/'.$field->PRDU_ID.'" class="btn btn-info btn-sm"><i class="fa fa-plus-square"></i> Product</a>
					</div></form>';
			}
			$data[] = $row;
		}
	...
	public function del(){
		$PRDU_ID = $this->input->post('PRDU_ID');
	...
	public function del_producer_x_product(){
		$PRDXP_ID = $this->input->post('PRDXP_ID');
		$PRDU_ID  = $this->input->post('PRDU_ID');
