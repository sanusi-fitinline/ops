...
class Incomebycs_m extends CI_Model {
    ...
    public function get_by_cs($FROM, $TO, $EXCLUDE_SHIPMENT, $USER_ID = null){
        if($EXCLUDE_SHIPMENT != 0){
            $this->db->select('tb_user.USER_NAME, IF(tb_order.ORDER_GRAND_TOTAL = 0, SUM(tb_order.ORDER_DEPOSIT - tb_order.ORDER_SHIPCOST), SUM(tb_order.ORDER_GRAND_TOTAL - tb_order.ORDER_SHIPCOST)) AS GRAND_TOTAL', FALSE);
        } else {
            $this->db->select('tb_user.USER_NAME, IF(tb_order.ORDER_GRAND_TOTAL = 0, SUM(tb_order.ORDER_DEPOSIT), SUM(tb_order.ORDER_GRAND_TOTAL)) AS GRAND_TOTAL', FALSE);
        }
        $this->db->from('tb_order');
        $this->db->join('tb_user', 'tb_user.USER_ID=tb_order.USER_ID', 'left');
        $this->db->where('tb_order.ORDER_STATUS >=', 2);    
