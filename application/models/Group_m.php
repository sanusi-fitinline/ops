...
class Group_m extends CI_Model {
	var $table = 'tb_group'; //nama tabel dari database
    var $column_search = array('GRP_NAME'); //field yang diizin untuk pencarian 
    var $order = array('GRP_NAME' => 'ASC'); // default order 
	...
	public function insert(){
		...
		$this->db->insert('tb_group', $this->db->escape_str($dataInsert));
	}
	...
	public function update($GRP_ID){
		...
		$this->db->where('GRP_ID', $GRP_ID)->update('tb_group', $this->db->escape_str($dataUpdate));
	}
	...
	public function delete($GRP_ID){
		...
		$this->db->where('tb_group.GRP_ID', $GRP_ID);
